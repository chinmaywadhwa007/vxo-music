...
export const FeaturedSingleBanner: React.FC = () => {
  ...
  const [audioFormat, setAudioFormat] = useState<'DOLBY' | 'STEREO' | '8D'>('DOLBY');
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-1 p-1 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
                {(['DOLBY', 'STEREO', '8D'] as const).map((mode) => (
                  <button
                    key={mode}
                    onClick={() => setAudioFormat(mode)}
                    className={`px-3 py-0.5 rounded-full text-[10px] font-mono font-bold transition-all duration-300 ${audioFormat === mode
                      ? 'bg-vexo-red text-white shadow-[0_0_12px_rgba(224,0,0,0.6)]'
